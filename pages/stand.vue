<template>
  <div class="px-4 py-8 max-w-7xl mx-auto flex flex-col items-center justify-center min-h-[70vh] gap-8">
    <div class="text-center space-y-4">
      <h2 class="text-4xl font-black tracking-tight text-primary">Bend a Stand</h2>
      <p class="text-gray-500 max-w-md mx-auto">Focus your strength! Only the area you touch will warp under the pressure. The more you bend, the more strength points you earn!</p>
    </div>

    <!-- Scoring Display -->
    <div class="flex flex-col items-center gap-1">
      <div class="text-xs font-bold uppercase tracking-widest text-gray-400">Strength Points</div>
      <div class="text-6xl font-black tabular-nums text-primary transition-all duration-75" :class="{'scale-110': isBending}">
        {{ Math.floor(score) }}
      </div>
    </div>
    ...
    <div 
      ref="gameArea"
      class="relative w-full max-w-md h-[400px] bg-gray-50 dark:bg-gray-900 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-800 flex items-center justify-center cursor-grab active:cursor-grabbing select-none overflow-hidden"
      @mousedown="startBend"
    ...
    >
      <div class="relative w-64 h-64">
        <!-- Layer 1: Static Base (Ghostly & Blurred) -->
        <svg fill="currentColor" class="absolute inset-0 w-full h-full text-gray-400 dark:text-gray-600 opacity-20 filter blur-[1px]" viewBox="0 0 201.862 201.862" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Layer 2: Locally Bent Part (Primary Colored & Glowing) -->
        <div 
          class="absolute inset-0 transition-all duration-75 ease-out will-change-transform drop-shadow-[0_0_8px_rgba(var(--color-primary-rgb),0.5)]"
          :style="{
        ...
        >
          <svg fill="currentColor" class="w-full h-full text-primary" viewBox="0 0 201.862 201.862" xmlns="http://www.w3.org/2000/svg">
            <path d="M100.931,201.861c-1.104,0-2-0.896-2-2V167.03l-36.146,34.282c-0.801,0.761-2.068,0.727-2.828-0.075c-0.76-0.801-0.727-2.067,0.075-2.827l38.899-36.893V80.284H37.464c-0.288,0-0.561-0.061-0.808-0.17c-0.234-0.104-0.452-0.253-0.639-0.45c-0.022-0.023-0.044-0.047-0.065-0.071c-0.155-0.179-0.272-0.377-0.354-0.586c0,0,0-0.003-0.001-0.005c-0.071-0.186-0.116-0.385-0.129-0.592v-0.002l0-0.003c-0.002-0.043-0.003-0.086-0.003-0.129V17.632c0-1.104,0.896-2,2-2H70.56V2c0-1.104,0.896-2,2-2h56.742c1.104,0,2,0.896,2,2v13.632h33.095c1.104,0,2,0.896,2,2v60.646c0,0.029,0,0.059-0.001,0.089c-0.001,0.002,0,0.003,0,0.005c-0.01,0.228-0.058,0.444-0.137,0.646c-0.091,0.231-0.228,0.449-0.409,0.641l0,0.001c-0.001,0.001-0.002,0.002-0.004,0.004l0,0c-0.001,0.002-0.003,0.003-0.004,0.004h0l-0.001,0.002c-0.189,0.197-0.409,0.347-0.644,0.448c-0.244,0.107-0.514,0.166-0.798,0.166h-61.466v81.233l38.741,36.742c0.801,0.76,0.835,2.026,0.075,2.827c-0.76,0.802-2.025,0.836-2.828,0.075l-35.988-34.132v32.831C102.931,200.966,102.036,201.861,100.931,201.861z M131.302,76.284h28.13l-28.13-27.048V76.284z M74.56,76.284h52.742V4H74.56V76.284z M42.531,76.284H70.56V50.068L42.531,76.284z M39.464,19.632v54.043L70.56,44.592v-24.96H39.464z M131.302,43.688l31.095,29.899V19.632h-31.095V43.688z M114.148,34.102H87.767c-1.104,0-2-0.896-2-2s0.896-2,2-2h26.381c1.104,0,2,0.896,2,2S115.253,34.102,114.148,34.102z M120.648,24.262H81.267c-1.104,0-2-0.896-2-2s0.896-2,2-2h39.381c1.104,0,2,0.896,2,2S121.753,24.262,120.648,24.262z" />
          </svg>
        </div>
      </div>
      ...
      <!-- Hint Layer -->
      <div class="absolute inset-x-0 bottom-4 text-center pointer-events-none transition-opacity duration-300" :class="{'opacity-0': isBending}">
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/50 dark:bg-black/50 backdrop-blur-sm text-[10px] font-bold uppercase tracking-wider text-gray-500">
          <UIcon name="i-heroicons-hand-raised" />
          Click & Pull to Bend
        </div>
      </div>
    </div>
    ...
    <!-- Info/Disclaimer -->
    <div class="flex items-center gap-2 p-2 px-4 rounded-lg bg-orange-100 dark:bg-orange-950 text-orange-700 dark:text-orange-300 text-xs font-semibold animate-pulse">
...
useHead({ title: 'bndstr — Bend a Stand' });

const score = ref(0);
const bendDepth = ref(0);
const mouseXRel = ref(128); // Center of 256px w-64
...
function updateMousePosition(clientX: number, clientY: number) {
  ...
  // We want coordinates relative to the 256x256 SVG container
...
function handleMouseMove(e: MouseEvent) {
  if (!isBending.value) {
    // Even when not bending, tracking mouse for the circle effect looks nice
  ...
  // Trigger squeal on significant change
  if (Math.abs(bendDepth.value - prevDepth) > 2 && Math.random() > 0.8) {
